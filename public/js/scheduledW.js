// NOTE:  Summary of events
/* This prgogram takes the user's id and compares them to contractors
attached to jobs, then it displays these jobs. When you click on them
The option to complete is inside. Saves the job status as complete which then
removes it from this view and reloads the page.
*/

//Completes the job by changing status
function complete(job){
   job.set("status", "completed");
   job.save(null, {
     success: function(job) {
       console.log("Job status is set to 'complete'...");
       location.reload();
     },
     error: function(job, error) {
       console.log("ERROR in job status set");
     }
   });
}//end of complete()

//Looks for jobs with your objectId in key contractor excluding the status "completed"
function pull(){
  var human = Parse.User.current();
  var Job = Parse.Object.extend("Job");
     var query = new Parse.Query(Job);
      query.equalTo("contractor", human);//8WKpZQ2a8z
      query.equalTo("status", "accepted");
      query.find().then(function(results) {
         for(var i=0; i< results.length;i++){

           var objectId = results[i].id,
           jobCity = results[i].get("jobCity"),
           jobState = results[i].get("jobState"),
           jobZip = results[i].get("jobZip"),
           subcategory = results[i].get("subcategory"),
           job = results[i];
           console.log(subcategory);
           var Subcategory = Parse.Object.extend("Subcategory");
              var subcat = new Parse.Query(Subcategory);
               subcat.equalTo("objectId", subcategory.id);
               subcat.find().then(function(result) {
                  for(var u=0; u< result.length; u++){
                    var sub = result[u].get("name");

                    var content = $("<div id=" + objectId + " class='well give-space job-item' data-toggle='modal' data-target='#jobModal'></div>");
                    content.append(" " + sub + " | " + jobCity + " | " + jobState + " | " + jobZip + " ")
                    .click(function(){
                      var jobItem = this.id;
                      modalJob(jobItem);

                      var modalFooter = $("<button id='complete" + objectId + "' style='float: left;' >Complete</button>");
                      $("#modalFooter").append(modalFooter);
                      $("#complete" + objectId + " ").on("click", function(){
                          complete(job);
                         });//End of click listener
                      });//end of on "click" method
                    $("#here").append(content);
                  }
                });
         }
       },function(){
         console.log("ERROR in pull()");
       });
}//End of pull

$(document).ready(function() {
  //Pull the user's information
  pull();
  //Make the modal for the jobs
  modalDeploy();
});
